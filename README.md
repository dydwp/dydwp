<div align="center">

![header](https://capsule-render.vercel.app/api?type=waving&color=0:6e6ef1,100:43cea2&height=200&section=header&text=Park%20Yong%20Je&fontSize=60&fontColor=ffffff&fontAlignY=38&desc=Backend%20Developer%20Java%20and%20Spring%20Boot&descAlignY=62&descSize=16&descColor=ffffff)

</div>

---

## 👋 Introduce

백엔드 중심으로 학습하며, 데이터를 다루고 서비스로 완성하는 과정에 흥미를 느끼는 개발자 **용제(dydwp)** 입니다.
Spring Boot 기반 웹 서비스 구축과 Python 기반 데이터 수집·분석을 함께 경험하며 실력을 쌓고 있습니다.

---

## 🛠 Tech Stack

<div align="center">

**Languages**

![Java](https://img.shields.io/badge/Java-000000?style=for-the-badge&logo=openjdk&logoColor=white)
![Python](https://img.shields.io/badge/Python-000000?style=for-the-badge&logo=python&logoColor=3776AB)

**Backend**

![Spring Boot](https://img.shields.io/badge/Spring_Boot-000000?style=for-the-badge&logo=springboot&logoColor=6DB33F)
![Spring Security](https://img.shields.io/badge/Spring_Security-000000?style=for-the-badge&logo=springsecurity&logoColor=6DB33F)
![Thymeleaf](https://img.shields.io/badge/Thymeleaf-000000?style=for-the-badge&logo=thymeleaf&logoColor=005F0F)

**Database**

![MySQL](https://img.shields.io/badge/MySQL-000000?style=for-the-badge&logo=mysql&logoColor=4479A1)
![JPA](https://img.shields.io/badge/JPA-000000?style=for-the-badge&logo=hibernate&logoColor=59666C)

**Data Collection & Analysis**

![Selenium](https://img.shields.io/badge/Selenium-000000?style=for-the-badge&logo=selenium&logoColor=43B02A)
![Jupyter](https://img.shields.io/badge/Jupyter-000000?style=for-the-badge&logo=jupyter&logoColor=F37626)

**Tools**

![Git](https://img.shields.io/badge/Git-000000?style=for-the-badge&logo=git&logoColor=F05032)
![GitHub](https://img.shields.io/badge/GitHub-000000?style=for-the-badge&logo=github&logoColor=white)
![VS Code](https://img.shields.io/badge/VS_Code-000000?style=for-the-badge&logo=visualstudiocode&logoColor=007ACC)
![IntelliJ IDEA](https://img.shields.io/badge/IntelliJ_IDEA-000000?style=for-the-badge&logo=intellijidea&logoColor=000000&labelColor=000000)

</div>

---

## 🧩 Projects

> 학습 과정에서 직접 기획하고 구현한 프로젝트들입니다.

### 🖥 Web

🏋️ **개인 맞춤형 운동 관리 웹 서비스, EasyFit** `Spring Boot` `MySQL` `JPA` `Thymeleaf` `Spring Security` `OAuth2`
> "원하는 운동 부위를 한눈에 파악하기 어렵고 초보자는 운동 방법을 알기 어렵다는 문제에서 출발해, 캘린더와 메모로 운동 기록을 관리할 수 있게 만든 개인 프로젝트입니다."

🧑‍💻 **주요 작업**

[기획, 설계, 풀스택 구현]

- MuscleWiki 스타일 SVG 신체 부위 지도와 캘린더 기반 운동 기록·메모 기능 설계 및 구현
- 정적 프론트엔드(HTML/CSS/Vanilla JS) 프로젝트를 Spring Boot REST API 기반으로 전환
- Entity → Repository → Service → Controller 계층형 아키텍처 적용
- Spring Security + OAuth2 기반 로그인 기능 구현
- UML 유스케이스 다이어그램, 화면설계서, ERD, MySQL DDL 등 설계 문서 작성

📌 프로젝트 링크: [easy-fit](https://github.com/dydwp/easy-fit)

---

### 📊 Data Collection & Analysis

🏟 **서울시 체육시설 공공서비스예약 데이터 크롤링, sports-crawling-project** `Python` `Selenium` `MySQL`
> "서울시 체육시설 공공서비스예약 사이트를 Selenium으로 동적 크롤링하여 MySQL에 적재하는 데이터 수집 프로젝트입니다."

🧑‍💻 **주요 작업**

[크롤러 설계, 데이터 적재]

- API 대신 URL 기반 방식으로 동적 페이지 크롤링 로직 설계
- Selenium을 활용한 예약 데이터 수집 및 전처리
- 수집한 데이터를 MySQL에 적재하는 파이프라인 구현

📌 프로젝트 링크: [sports-crawling-project](https://github.com/dydwp/sports-crawling-project)

---

🏫 **School Zone 지정 현황 데이터 분석, school-zone-analysis** `Python` `Jupyter Notebook`
> "전국 시·도별 School Zone(어린이보호구역) 지정 현황 데이터를 분석한 프로젝트입니다."

🧑‍💻 **주요 작업**

[데이터 분석]

- 전국 시·도별 어린이보호구역 지정 현황 데이터 수집 및 정제
- Jupyter Notebook을 활용한 지역별 현황 비교 분석

📌 프로젝트 링크: [school-zone-analysis](https://github.com/dydwp/school-zone-analysis)
